...
class Damage:
    def __init__(self, damage_table):
        ...
        self.dh = (self.h_max - self.h_min) / self.water_level.size
    ...
    def calc_damage(self, water_level):
        ...
        for div in range(water_level.shape[1]):
            for s, storm in enumerate(water_level[:, div]):
                index = floor((water_level[s, div] - self.h_min) / self.dh)

                damage[s, div] = self.damage_table[str(div)][index] + (water_level[s, div] - self.water_level[index])*(self.damage_table[str(div)][index + 1] - self.damage_table[str(div)][index]) / self.dh
        return damage
