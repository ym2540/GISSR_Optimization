...
def objective(x,SVf1,SVf2,SVf3,SVf4,SVf5,SVf6,SVf7,SVf8,SVf9,SVf10,SVf11,SVf12,SVf13,SVf14,SVf15,SVf16,SVf17,SVf18,SVf19,SVf20,
    ...
    # wall_cost = (22195*wall_h-16553)*wall_id.size*100/((1+0.05)**(wall_year-2020))
    # wall_cost = (22195*wall_h-16553)*(wall_end-wall_start)*100/((1+0.05)**(wall_year-2020))
    #wall_cost = 109360*wall_h*(np.sum(x[0:-1]))*100
    #wall_cost = 109360*100*np.sum(x)
    
    wall_cost = 49212*100*np.sum(x)
    ...
    for N in range(numiter):
        ...
        damage_loss_c,inop_util_c,inop_tran_c,cost_util_c,cost_tran_c, df_cost_direct_sum_div_c  = damage.dmg_cost_vector(fld_h_c, flh, ftm, fid_dmg, crtcl_elv, bldgasst, bsmt, comarea, fldpln, numfl, bldgclass, div, df_fire, df_poli, df_hosp, df_nurs, df_schl, df_eoc, df_res11, df_res11b, df_res12, df_res12b, df_res2, df_res3, df_res3b, df_htl, df_com1, df_com2, df_com3, df_com4, df_com5, df_com7, df_com8, df_com9, df_com10, df_ind1, df_ind2, df_ind3, df_ind4, df_ind5, df_ind6, df_agr, df_rel, df_gov, df_elec,H_dmg,H_dmg1,N)
        damage_loss_w,inop_util_w,inop_tran_w,cost_util_w,cost_tran_w, df_cost_direct_sum_div_w  = damage.dmg_cost_vector(fld_h_w, flh, ftm, fid_dmg, crtcl_elv, bldgasst, bsmt, comarea, fldpln, numfl, bldgclass, div, df_fire, df_poli, df_hosp, df_nurs, df_schl, df_eoc, df_res11, df_res11b, df_res12, df_res12b, df_res2, df_res3, df_res3b, df_htl, df_com1, df_com2, df_com3, df_com4, df_com5, df_com7, df_com8, df_com9, df_com10, df_ind1, df_ind2, df_ind3, df_ind4, df_ind5, df_ind6, df_agr, df_rel, df_gov, df_elec,H_dmg,H_dmg1,N)
