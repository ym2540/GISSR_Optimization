...
def objective2(heights):
    
    x = np.concatenate((np.ones(41)*heights[0],np.ones(40)*heights[1],np.ones(41)*heights[2],np.ones(41)*heights[3]))
...
wall_init = [40.,60.,1.0]
x_opt = scipy.optimize.minimize(objective3, wall_init, method='Nelder-Mead', tol = 10.0e6, constraints = {'type': 'ineq', 'fun': lambda x:  x[1] - x[0]}, options = {'disp': True, 'return_all': True})
print(x_opt)           
